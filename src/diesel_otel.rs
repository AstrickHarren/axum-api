...
pub trait RunQueryDsl<Conn>: Sized {
    ...
    {
        <Self as diesel_async::RunQueryDsl<Conn>>::load_stream(self, conn)
            .instrument(make_otel_span())
    }
    ...
    where
        ...
        Self: methods::LoadQuery<'query, Conn, U> + 'query,
    {
        <Self as diesel_async::RunQueryDsl<Conn>>::get_result(self, conn)
            .instrument(make_otel_span())
    }
    ...
    where
        ...
        Self: methods::LoadQuery<'query, Conn, U> + 'query,
    {
        <Self as diesel_async::RunQueryDsl<Conn>>::get_results(self, conn)
            .instrument(make_otel_span())
    }
